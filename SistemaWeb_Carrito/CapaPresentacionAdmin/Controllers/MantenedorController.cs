    ...
    public class MantenedorController : Controller
    {
        ...
        public JsonResult GuardarProducto(string objeto, HttpPostedFileBase archivoImagen) /*De este json se puede controlar que mas ver, igualar elementos, etc*/
        {
           ...
           /*Resultado = Va a permitir almacenar cualquier tipo de resultado (en este caso int o booelan, dependiendi si es creacion o edicion)*/
            ...
            if (int.TryParse(oProducto.PrecioTexto, System.Globalization.NumberStyles.AllowDecimalPoint, new CultureInfo("es-MX"), out precio))
            {
                /*Si esta todo correcto
                 * Ahora si pasa el valor convertido a Precio de la entidad EN_Productos
                 */
                oProducto.Paginas = precio;
            }
            else /*Si hay algun problemas*/
            {
                return Json(new { operacionExitosa = false, mensaje = "El formato de las paginas debe ser ##"}, JsonRequestBehavior.AllowGet);
            }

            if (oProducto.IdLibro == 0)/*Es decir, si el id es 0 en inicio (el valor es 0 inicialmente) significa que es
             una categoria nueva, por lo que se ha dado dando clic con el boton de crear*/
            {
                ...
                {
                    oProducto.IdLibro = idProductoGenerado;/*Toma el id*/
                }
                else
                {
            ...
            if (operacionExitosa)/*Si la operacion es true  pasamos  actualizar una ruta de imagen, guardar una imagen*/
            {
                if(archivoImagen != null)
                {
                    ...
                    string nombreImagen = string.Concat(oProducto.IdLibro.ToString(), extensionImagen);
                    ...
                    }else /*En el caso de que la imagen no haya sido guardada con exitos*/
                    {
                        ...
                        mensaje = "Se guardó el libro pero hubo problemas con la imagen";
                    }
                }
            }
            return Json(new { operacionExitosa = operacionExitosa, idGenerado = oProducto.IdLibro, mensaje = mensaje }, JsonRequestBehavior.AllowGet);
            //return Json(new { resultado = resultado, mensaje = mensaje }, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult ImagenProducto(int id)
        {
            ...
            EN_Producto oProducto = new RN_Producto().Listar().Where(p => p.IdLibro == id).FirstOrDefault();
